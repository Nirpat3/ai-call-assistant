...
import { apiGateway } from './ServiceRegistry';
...
export class CallWorkflowService {
  async processIncomingCall(
    ...
    const contact = await this.checkContactPassthrough(callerNumber, organizationId);
    if (contact) {
      return contact;
    }

    // Step 2: Check business hours
    const businessHoursCheck = await this.checkBusinessHours(organizationId);
    ...
    const aiConfig = await apiGateway.getAIConfig(organizationId);
    if (!aiConfig) {
      return {
        action: 'ai_greeting',
    ...
    if (aiConfig.phoneTree && aiConfig.phoneTree.enabled) {
      return {
        action: 'phone_tree',
        greeting: aiConfig.greeting,
        phoneTreeOptions: aiConfig.phoneTree.options,
  ...
  private async checkContactPassthrough(
    callerNumber: string,
    organizationId: string
  ): Promise<CallWorkflowResult | null> {
    const contact = await apiGateway.getContactByPhone(callerNumber, organizationId);
    if (!contact) return null;

    const contactRoutes = await apiGateway.getContactRoutesForPhone(callerNumber, organizationId);
    ...
    for (const route of contactRoutes) {
      ...
      if (route.businessHoursOnly) {
        const businessHours = await this.checkBusinessHours(organizationId);
  ...
  private async checkBusinessHours(organizationId: string): Promise<{
    isOpen: boolean;
    ...
    const daySchedule = aiConfig.businessHours[currentDay];
  ...
  async handlePhoneTreeSelection(
    selection: string,
    organizationId: string
  ): Promise<CallWorkflowResult> {
    const aiConfig = await apiGateway.getAIConfig(organizationId);
    if (!aiConfig?.phoneTree) {
      return {
        action: 'ai_greeting',
    ...
    const selectedOption = aiConfig.phoneTree.options.find(
      (option: any) => option.key === selection
    );
...
<Response>
  ...
  <Redirect>/api/twilio/ai-greeting</Redirect>
      ...
      case 'ai_greeting':
        return `<?xml version="1.0" encoding="UTF-8"?>
<Response>
  <Gather action="/api/twilio/ai-gather" method="POST" input="speech" timeout="5">
...
<Response>
  ...
  <Record action="/api/twilio/voicemail" method="POST" maxLength="120" transcribe="true"/>
</Response>`;
...
export const callWorkflowService = new CallWorkflowService();